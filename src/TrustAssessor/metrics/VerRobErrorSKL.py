...
class VerifiedErrorSKLTree(Metric):
    """Verified error of a decision-tree sklearn-based model on the provided dataset (dataX, dataY) using the ART package.

    This metric is typically used to verify the robustness of the classifier on the provided dataset. Although it is a cost function, its assessment is inverted so it can be treated as the rest of metrics (i.e., as a percentage).
